...
- **Funnel de cobro (4 etapas)**: cada caso tiene una etapa que mueve el gestor
  desde la ficha — `pendiente` → `en_gestion` → `cobrado` / `incobrable`.
  Enfocado en **cobrar** (no en negociar). Se asigna un **gestor** a cada caso.
  Etapa y gestor son filtrables y visibles en la tabla.
- **Embudo de cobro**: los casos por etapa, medido en dinero (deuda vencida y
  recobrado), no en nº de casos; los casos ya cobrados permanecen en su etapa.
- **Descuento por pronto pago**: desde la ficha se ofrece un descuento por pagar
  ya (slider de %), calcula el importe a pagar y su ahorro, con fecha límite; la
  oferta queda registrada en el historial de gestiones.
...
> El **estado de morosidad** (por tiempo, automático) y la **etapa de cobro**
> (el trabajo del caso, la mueve el gestor) son dos ejes distintos y
> complementarios. A diferencia de Hofmann (funnel de negociación), aquí el
> funnel está orientado a **cobrar**.
